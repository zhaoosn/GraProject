...
export default {
  ...
  mounted () {
    ...
    dashboardApi.index().then(re => {
      ...
      _this.echartsUserAction.setOption(this.option('用户活跃度', '{b}日<br/>{c}热度', response.mothDayText, response.mothDayUserActionValue))
      _this.echartsQuestion.setOption(this.option('题目月数量', '{b}日<br/>{c}题', response.mothDayText, response.mothDayDoExamQuestionValue))
      this.loading = false
    })
  },
  methods: {
