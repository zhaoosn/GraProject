  ...
  <div style="margin-top: 10px" class="app-contain">
    ...
    <div style="margin: 10px 0">
      <div style="padding: 10px 0; border-bottom: 1px dashed #ccc" v-for="item in tableData" :key="item.id">
        <div class="pd-10" style="font-size: 20px; color: #3F5EFB; cursor: pointer" @click="$router.push('/article/articleDetail?id=' + item.id)">{{ item.name }}</div>
  ...
  export default {
    ...
    methods: {
      ...
      imgAdd(pos, $file) {
        ...
        axios({
          url: 'http://localhost:8000/student/file/upload',
          method: 'post',
          data: formData,
